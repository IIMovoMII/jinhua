...
- 用户说“记住这个”“以后都这样”“沉淀一下”“写进 Skill”“更新 Skill”“所有项目都这样”。
...
| 场景 | 动作 |
| --- | --- |
| 用户纠正了推理、验证或工作流程 | 先跑 `cycle`；如果能写成 `trigger` + `action`，再考虑 `log-signal`。 |
...
唤醒机制故意保持很轻：宿主 agent 平时只看到 Skill 的元信息（metadata），任务匹配后才加载正文并运行 `cycle`。除非用户明确要求实现，否则不要加入后台进程、hook 或机器学习。
