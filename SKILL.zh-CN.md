...
- 同一项目的同一聚类达到 2 条信号时，不看强度，先进入“仅项目规则就绪”；这时只能提议写入当前项目的 `project_rule`。
- 达到 3 条信号，或总强度达到 5 时，进入原有完整本地就绪，可以继续按正常落点阶梯判断。

两条信号路径的提案目标和实际修改目标都必须位于当前项目内。明确要求立即沉淀或紧急高成本失败可以使用 `log-signal --immediate`。项目规则被采纳后，原信号不会被消费或停用，仍可作为压缩证据参与后续跨项目累计。

核心命令会按当前阈值重新检查已有活跃聚类，因此历史证据无需重写或重新记录，也能按新规则进入就绪。
...
如果只是两条同项目信号触发的“仅项目规则就绪”，确认门只显示“项目规则、拒绝、修订”，不能直接升级为 `skill_patch` 或 `personal_global_skill`。
