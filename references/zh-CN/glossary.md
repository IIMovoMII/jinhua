...
| `reject-proposal` | 拒绝本地提案 | 用户选择 `拒绝(No)` 或 `修订(Revision)` 后记录拒绝/修订，并进入冷却。 |
...
| `global-reject` | 拒绝全局提案 | 用户选择 `拒绝(No)` 或 `修订(Revision)` 后记录全局拒绝/修订。 |
...
| `user_gate` | 用户确认门 | 现在是带落点的确认：`项目规则(project_rule)` / `增强已有 Skill(skill_patch)` / `个人全局 Skill(personal_global_skill)` / `拒绝(No)` / `修订(Revision)`。 |
