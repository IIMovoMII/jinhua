...
- 新增两级本地就绪：同项目同类经验达到 2 条时，不看强度，只允许提出当前项目规则；原来的 3 条信号或总强度 5 继续提供完整落点判断。
- 新增跨项目重复路径：同类方法在至少 2 个项目累计 3 条证据后进入全局就绪，不要求强度；原普通阈值和强证据快速路径继续保留。
- 项目规则采纳不再被理解为“消费证据”：原信号保持活跃并可继续参与一次性全局导入，项目内先沉淀不会阻断后续全局晋升。
- 项目专用提案增加 `project_only` 硬约束，确认门只显示项目规则、拒绝和修订，不能在两条证据阶段直接升级为 Skill。
- 非 Hook 核心命令会幂等重算历史活跃聚类，使已有证据立即适用新阈值，不需要迁移、清零或重新记录信号。
- `cycle` 会按每个就绪聚类的实际范围展示对应确认门，不再给项目专用聚类误报完整五选项。
- 将固定每 8 轮的第三道回顾从 Codex 可见的 Stop `HookPrompt` 迁移到 `UserPromptSubmit.additionalContext`；三道逻辑闸门保留，周期回顾不再产生额外模型继续或用户可见内部提示。
- 删除 `codex-stop`、Stop wrapper、Stop Hook 清单项和周期 ticket；旧触发运行态会在下一次输入 Hook 中清理废弃字段。
...
- 项目、会话和回合状态只从宿主权威字段读取；未知 payload 不再改变调用保护状态。
- Codex 使用 `turn_id`、Claude Code 兼容 `prompt_id`；同一回合的多个 Jinhua 子命令只记录一个保护事件。
- 新增“提到命令”和“执行命令”的反例测试，并覆盖伪造 `cwd`、session、turn 和任意嵌套文本。
