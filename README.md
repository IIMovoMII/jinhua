...
命中时只向当前正常模型调用加入一句短内部提示。它还会只读检查现有就绪聚类和待确认门、按会话累计不同用户回合，并承载第三道固定周期回顾。它不运行 `cycle`，不迁移核心数据，不写 signals/proposals，也不保存用户原文。
...
同一个 `UserPromptSubmit` Hook 按单个会话固定每 8 个不同用户回合，向当前正常模型调用的 `additionalContext` 加入一次极短回顾，要求 Agent 检查本轮及此前对话。重复的同一 `turn_id` 不会再次注入。

Codex 会把 Stop Hook 的 `decision:block + reason` 转成用户可见的 `HookPrompt`，因此 Jinhua 不再用 Stop 传输周期提示。第三道门仍然固定执行，但不产生额外模型继续，也不向用户展示内部提示。
...
```text
同一项目同类信号数 >= 2
→ 仅项目规则就绪，不看强度，只能提议 project_rule
...
“就绪”表示证据允许形成提案，不代表已经改规则。两条同项目信号产生的提案只能写入当前项目；达到完整阈值后才恢复原有三种落点判断。下一次输入侧 ready-attention 会把它带回 Agent 注意力，由 Agent 创建完整提案或给出具体跳过原因。

项目规则被采纳后，原信号不会归零、删除或停用。它们仍会压缩导入全局层，与其他项目后续出现的同类信号继续累计。

运行 `cycle` 等核心命令时，CLI 会幂等重算已有活跃聚类的就绪状态；历史信号不需要重新记录。
...
例如：B 项目出现两次并先采纳为项目规则，C 项目再出现一次同类方法；三条证据仍会合并，使该方法进入全局就绪，但仍需生成提案并经过用户确认。
...
两条同项目信号触发的项目专用提案只显示：`项目规则(project_rule)`、`拒绝(No)`、`修订(Revision)`。
...
- 固定每 8 轮只给当前正常模型调用增加一句极短内部回顾，不产生额外模型继续。
